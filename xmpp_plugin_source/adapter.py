...
from .xmpp_utils import guess_content_type, parse_bool, set_nested_config_value
...
class XMPPAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.allow_all_users = parse_bool(
            os.getenv("XMPP_ALLOW_ALL_USERS") or extra.get("allow_all_users"), True
        )
        raw_allowed = os.getenv("XMPP_ALLOWED_USERS") or extra.get("allowed_users")
        self.allowed_users: Optional[List[str]] = None
        if raw_allowed:
            if isinstance(raw_allowed, str):
                self.allowed_users = [u.strip() for u in raw_allowed.split(",") if u.strip()]
            else:
                self.allowed_users = list(raw_allowed)
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        return {"chat_id": chat_id, "title": chat_id}
    ...
    async def send_voice(self, chat_id: str, file_path: str, metadata: Optional[Dict[str, Any]] = None) -> SendResult:
        return await self._send_file_by_upload(chat_id, file_path, content_type_hint="audio/ogg")

    async def send_image_file(self, chat_id: str, file_path: str, caption: str = "", metadata: Optional[Dict[str, Any]] = None) -> SendResult:
        return await self._send_file_by_upload(chat_id, file_path, content_type_hint="image/png", caption=caption)
    ...
    async def _send_file_by_upload(
        ...
        try:
            ...
            upload_plugin = self.client.plugin.get("xep_0363")
    ...
    async def _on_message(self, msg: Message):
        try:
            logger.warning("XMPP: _on_message fired type=%s from=%s body=%r", msg.get("type", ""), msg.get_from(), msg.get("body", "")[:60])
            ctx = InboundContext(adapter=self, msg=msg)
            await self._inbound_pipeline.run(ctx)
        except Exception as exc:
            logger.exception("XMPP: _on_message crashed: %s", exc)
            raise
    ...
    def _send_displayed_marker(self, to_jid: str, message_id: str) -> None:
        if self.client is None:
            return
        marker_plugin = self.client.plugin.get("xep_0333")
    ...
    async def _publish_avatar(self) -> None:
        if not self.avatar_path or self.client is None:
            return
        try:
            from slixmpp.plugins.xep_0084 import avatars
            ...
            avatar_hash = avatars.compute_id(avatar_data)
            vcard = self.client.plugin.get("xep_0054")
            if vcard is not None:
                await vcard.publish_avatar(avatar_data)
                await vcard.publish_avatar_metadata(avatar_hash, avatar_data)

            avatar_plugin = self.client.plugin.get("xep_0084")
            if avatar_plugin is not None:
                await avatar_plugin.publish_avatar(avatar_data)
                await avatar_plugin.publish_avatar_metadata(
                    {
                        "id": avatar_hash,
                        "type": "image/png",
            ...
            logger.info("XMPP: avatar published")
        except Exception as exc:
            logger.warning("XMPP: failed to publish avatar: %s", exc)
